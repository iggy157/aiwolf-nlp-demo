...
class Agent:
    ...
    def initialize(self) -> None:
        ...
        match model_type:
            # openai: 商用OpenAI / vllm: OpenAI互換ローカルエンドポイント
            # どちらも ChatOpenAI を使い、base_url の有無だけで切り替える（HANDOFF §8）。
            case "openai" | "vllm":
                openai_conf = self.config["openai"]
                # base_url は config の openai.base_url を最優先。
                # 環境変数 OPENAI_BASE_URL は vllm のときだけフォールバックに使う。
                # （商用openai は OPENAI_BASE_URL が残っていても無視 → vLLM用URLの取り違え防止）
                base_url = str(openai_conf.get("base_url") or "").strip() or None
                if base_url is None and model_type == "vllm":
                    base_url = str(os.environ.get("OPENAI_BASE_URL") or "").strip() or None
                if base_url:
                    # vLLM 等の OpenAI 互換はダミーキーで可
                    api_key = (
                    ...
                    # 商用OpenAIはキー必須（未設定なら従来どおりエラー）
                    api_key = os.environ["OPENAI_API_KEY"]
                self.llm_model = ChatOpenAI(
                    model=str(openai_conf["model"]),
                    temperature=float(openai_conf["temperature"]),
